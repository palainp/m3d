...
static void _echangerPoint2d(t_triangle2d *t, size_t i, size_t j)
{
	assert(t!=NULL);
	assert(i<3);
	assert(j<3);
...
static void _trierSommets(t_triangle2d *t)
{
	assert(t!=NULL);
...
static void _majMinMax(t_surface *s, t_triangle2d *t, size_t i, size_t j)
{
	assert(s!=NULL);
	assert(t!=NULL);
	assert(i<j);
